...
class TnbToolsOperatorExportTextureDirectionsAsImage2(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    bl_idname       = "tnb_tools.operator_export_texture_directions_as_image_2"
    bl_label        = "tnb_tools_export_texture_directions_as_image_2"
    ...
    def output_tnb_direction_image(self, context):
        ...
        for cur_face in mesh_edit.faces:
            ...
            circle_center = v_0 - (w_0 * max_geodesic_distance) * cur_face_direction_normalized

            # # That this direction is valid at v1 which is at distance w1
            # if w_1 == w_2:
            #     v_start = v_1
            #     weight_start = w_1
            ...
            # # Then, compute the center of the 3D circle
            # circle_center = v_start - (weight_start * max_geodesic_distance) * cur_face_direction_normalized

            # correction_factor = 1.015
            # circle_center = v_start - ((weight_start*correction_factor) * max_geodesic_distance) * cur_face_direction_normalized
            # # 1.015 is an empirical correction factor computed as the ratio of
            # # the geodesic distance returned by geodoodle and the actual distance
            # # between a source vertex and its closest neighbour
...
def register():
    bpy.utils.register_class(TnbToolsOperatorExportTextureDirectionsAsImage2)


def unregister():
    bpy.utils.unregister_class(TnbToolsOperatorExportTextureDirectionsAsImage2)
...
if __name__ == "__main__":
    ...
    # test call
    bpy.ops.tnb_tools.operator_export_texture_directions_as_image_2('INVOKE_DEFAULT')